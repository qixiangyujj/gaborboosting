...
void MatFunctions::poolloadfromsub(const char *filepath, const char *name, int iter, int NumSub, CvGaborFeaturePool *pool)
{
  for(int i = 0; i < NumSub; i++)
  {
    char * filename = new char[50];
    sprintf( filename, "%s%s_%d_%d.txt", filepath, name, iter, i );
    pool->addfile( filename );
    delete [] filename;
  }
  printf("%d features are loaded from files.\n", pool->getSize());
}
